...
def create_data_lists(kaist_path, output_folder):
    ...
    :param kaist_path: path to the 'KAIST_PD' folder
    ...
    for rgb, lwir in zip(train_rgb_images, train_lwir_images):
        train_total_images.append(rgb)
        train_total_images.append(lwir)

    # # Save to file
    with open(os.path.join(output_folder, 'TRAIN_rgb_images.json'), 'w') as j:
        json.dump(train_rgb_images, j)
    with open(os.path.join(output_folder, 'TRAIN_lwir_images.json'), 'w') as j:
        json.dump(train_lwir_images, j)
